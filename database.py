...
async def get_full_order_details(order_id: str):
    return await _db_query("SELECT order_id, user_id, order_type, package_name, credit_cost, payment_method, status, screenshot_file_id, delivery_info, created_at FROM orders WHERE order_id = ?", (order_id,), fetchone=True)
